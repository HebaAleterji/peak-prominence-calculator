test_that("detect_peaks correctly identifies peaks in 2D data", {
  ...
  expect_equal(result_list[[1]]$Height, c(12, 12)) # Expected heights for row 1

  expect_equal(result_list[[2]]$Position, c(2, 6, 7)) # Expected positions for row 2
  expect_equal(result_list[[2]]$Height, c(8, 11, 14)) # Expected heights for row 2
})
